...
@file: 东方头条.py
@description：东方头条新闻客户端爬虫
"""
"""
修改@201907 部分功能增加随机等待时间
增加将已阅读新闻存放到文本文件减少重复阅读(保留最新100条)
@author: cqmoyf
@contact: cqmoyf@hotmail.com
@site: http://www.fanqiedashu.com
@software: PyCharm
@time: 7/15/19 16:56
"""
...
class DongFangTouTiao(object):
    ...
    def __init__(self):
        ...
        # 跳过的页数
        self.skip_page = 0
    def write_t(self, news_titles):
        with open("news.txt", "w") as f:
            for __title in news_titles:
                f.write(__title)
                f.write('\n')
    ...
    def run(self):
        ...
        # 5.加载已阅读新闻标题
        self.news_titles = self.read_t()

        while True:
            self.watch_news_recommend()

            print('查看一页完成，继续查看下一页的新闻。')
            self.write_t(self.news_titles)
            ...
            # 滑动下一页的新闻
            # swipe_start = [round(random.uniform(0.4, 0.6),2), round(random.uniform(0.7, 0.9),2)]
            # swipe_end = [round(random.uniform(0.4, 0.6),2), round(random.uniform(0.3, 0.5),2)]
            # duration_time = round(random.uniform(0.6, 1.5),2)
            # poco.swipe(swipe_start, swipe_end, duration=duration_time)
            self.__swipe()
    ...
    def watch_news_recommend(self):
        ...
        # 1.推荐的所有新闻元素
        lv_elements = poco('com.songheng.eastnews:id/fu').children()
        ...
        for news_element in lv_elements:
            ...
            # 2.新闻标题
            if news_element.offspring('com.songheng.eastnews:id/ot').exists():
                news_title = news_element.offspring('com.songheng.eastnews:id/ot')
            elif news_element.offspring('com.songheng.eastnews:id/kk').exists():
                news_title = news_element.offspring('com.songheng.eastnews:id/kk')
            else:
                news_title = news_element.offspring('com.songheng.eastnews:id/akqwerewq')

            # 作者
            author_element = news_element.offspring('com.songheng.eastnews:id/a4d')
            ...
            # 评论拿金币和发表按钮
            comments_with_coins = poco('com.songheng.eastnews:id/ln')
            submit_btn_element = poco("com.songheng.eastnews:id/m6").offspring('com.songheng.eastnews:id/vw')
            # 记录时长的标识
            # 不存在就直接返回
            red_coin_element = poco('com.songheng.eastnews:id/ap7')
            ...
            if comments_with_coins.exists() and comments_with_coins.get_text() == '评论拿金币':
                ...
                if news_type == 0:
                    while True:
                        __sleep_time = random.randint(1, 5)
                        print("循环-滑动查看内容.等待{}秒".format(__sleep_time))
                        sleep(__sleep_time)
                        ...
                        read_time = 30+random.randint(1, 11)
                        if interval_time >= read_time:
                            print('阅读{}秒新闻完成'.format(read_time))
                            break
                        self.__read_key_news()
                # 视频
                elif news_type == 1:
                    while True:
                        ...
                        interval_time = (newtime - oldtime).seconds
                        if interval_time >= read_time:
                            print('观看{}秒视频完成'.format(read_time))
                            break
                        self.__read_key_news()
            else:
                print('这是一篇没有金币的文章！')
    ...
    def __video(self):
        ...
        :return:
        """
        poco(name = 'com.songheng.eastnews:id/adn', text='视频').click()
        ...
        while True:
            ...
            for video_element in video_elements:
                # 1.标题元素
                video_title_element = video_element.offspring('com.songheng.eastnews:id/ot')
                # 播放按钮
                video_play_element = video_element.offspring('com.songheng.eastnews:id/ph')
                ...
                # 3.标题
                video_title = video_element.offspring('com.songheng.eastnews:id/ot').get_text()
    ...
    def mini_video(self):
        ...
        :return:
        """
        poco(name='com.songheng.eastnews:id/js',text='视频').click()
        poco(text='小视频').click()

        # 加载出列表元素,点击第一项进入
        poco('com.songheng.eastnews:id/a0q').child('com.songheng.eastnews:id/fu').wait_for_appearance(60)
        poco('com.songheng.eastnews:id/a0q').child('com.songheng.eastnews:id/fu').children()[0].click()
    ...
    def __swipe(self, up_or_down = True):
        """
        随机滑动屏幕(滑动新闻,滑动页面)
        :param up_or_down: true：往上滑动；false：往下滑动【慢慢滑动】 默认向上滑动
        :return:
        """
        swipe_start = [round(random.uniform(0.4, 0.6), 2), round(random.uniform(0.7, 0.9), 2)]  # 屏幕下方点
        swipe_end = [round(random.uniform(0.4, 0.6), 2), round(random.uniform(0.3, 0.5), 2)]  #屏幕上方点
        duration_time = round(random.uniform(0.8, 1.5), 2)
        # print(swipe_start,swipe_end,duration_time)
        if up_or_down:
            poco.swipe(swipe_start, swipe_end, duration=duration_time)
        else:
            poco.swipe(swipe_end, swipe_start, duration=duration_time)
    ...
    def get_news_type(self, news_element):
        ...
        video_element = news_element.offspring('com.songheng.eastnews:id/kk')
    ...
    def __pre_and_skip_ads(self):
        ...
        try:
            poco('com.songheng.eastnews:id/anx').wait_for_appearance(5)
        except Exception as e:
            print('等待广告元素异常')
            print(e)
        ...
        # 2.等到到达主页面
        poco('com.songheng.eastnews:id/fu').wait_for_appearance(120)
    ...
    def __read_key_news(self):
        ...
        """
        # 对于弹出来的要闻对话框，需要处理
        key_news_element = poco(name='com.songheng.eastnews:id/wn', text='立即查看')
    ...
    def get_top_title_coin(self):
        ...
        :return:
        """
        get_coin_element = poco(name='com.songheng.eastnews:id/arc', text="领取")
    ...
    def __back_to_list(self):
        ...
        """
        print('准备回到首页')
        while not poco('com.songheng.eastnews:id/fu').exists():
            print('回退一次')
            self.__back_keyevent()
